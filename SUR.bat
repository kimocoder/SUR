...
title %app_name% [v1.3] [64bits] %authors%
...
::<---commands
echo ---- System extraction finished: %time% >> log.txt
set ENDTIME=%TIME%
call :time
...
echo         *****************************************************
echo.
%cecho%  	-{0a}Extracting payload.bin{#} format...
echo.
echo.
echo.
echo ---- Payload.bin format found: >> log.txt
echo      Extracting system images... >> log.txt
...
echo.
echo.
echo ---- Extracting sparse system format >> log.txt
::--->commands
...
if exist %system%\etc\selinux\plat_file_contexts xcopy %system%\etc\selinux\plat_file_contexts 01-Project\1-Sources /y >nul
if exist %vendor%\etc\selinux\vendor_file_contexts type %vendor%\etc\selinux\vendor_file_contexts >> 01-Project\1-Sources\plat_file_contexts
if exist %vendor%\etc\selinux\nonplat_file_contexts type %vendor%\etc\selinux\nonplat_file_contexts >> 01-Project\1-Sources\plat_file_contexts
if exist 01-Project\1-Sources\plat_file_contexts !busybox! sort -u < 01-Project\1-Sources\plat_file_contexts >> 01-Project/1-Sources/file_contexts
if exist 01-Project\1-Sources\file_contexts !busybox! rm -rf 01-Project\1-Sources\plat_file_contexts >nul 2>nul
::--->commands
...
if exist 01-Project\1-Sources\file_contexts if exist 01-Project\1-Sources\file_contexts (goto rebuild_yes) else (goto file_contexts_finder)
:rebuild_yes
...
echo.
echo.
echo      Repacking to: %format% format... >> log.txt
::--->commands
if exist %system%\build.prop bins\make_ext4fs -L system -T 2009110000 -S 01-Project\1-Sources\file_contexts -C 01-Project\1-Sources\fs_config -l %size% -a system %~dp0\02-Output\system.img 01-Project\system\ >nul 2>nul
...
echo      Repacking to: %format% format... >> log.txt
::--->commands
if exist %system%\build.prop bins\make_ext4fs -s -L system -T 2009110000 -S 01-Project\1-Sources\file_contexts -C 01-Project\1-Sources\fs_config -l %size% -a system %~dp0\02-Output\system.img 01-Project\system\ >nul 2>nul
...
::--------------new.dat part--------------::
timeout /t 1 /nobreak >nul
if exist %~dp0\02-Output\system.img bins\img2sdat %~dp0\02-Output\system.img  -o %~dp0\02-Output -v %version% >nul
timeout /t 1 /nobreak >nul
if exist %~dp0\02-Output\vendor.img bins\img2sdat %~dp0\02-Output\vendor.img  -o %~dp0\02-Output -v %version% -p vendor >nul
timeout /t 1 /nobreak >nul
...
:build_vendor
bins\make_ext4fs -s -L vendor -T 2009110000 -S 01-Project\1-Sources\file_contextsv -C 01-Project\1-Sources\fs_configv -l %vsize% -a vendor %~dp0\02-Output\vendor.img 01-Project\vendor\ >nul 2>nul
if exist 02-Output\vendor.img echo      Repacking vendor.img [OK] >> log.txt
goto:eof
:build_vendor2
bins\make_ext4fs -L vendor -T 2009110000 -S 01-Project\1-Sources\file_contextsv -C 01-Project\1-Sources\fs_configv -l %vsize% -a vendor %~dp0\02-Output\vendor.img 01-Project\vendor\ >nul 2>nul
...
if exist 01-Project\1-Sources\boot.img bins\rmverity -s 01-Project\1-Sources\boot.img >> 01-Project\temp\boot_status
